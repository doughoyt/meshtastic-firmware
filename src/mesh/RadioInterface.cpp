...
{
#ifdef DEBUG_PORT
    std::string out = DEBUG_PORT.mt_sprintf("%s (id=0x%08x fr=0x%04x to=0x%04x, WantAck=%d, HopLim=%d Ch=0x%x", prefix, p->id,
                                            p->from & 0xffff, p->to & 0xffff, p->want_ack, p->hop_limit, p->channel);
